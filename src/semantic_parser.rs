...
impl Grammar {
    ...
    pub fn semantic_parse_text_from_start(
        &self,
        text: &str,
        ...
        for rule in public_rules {
            let results = rule.semantic_parse(0, &text_tokens, rule_heap, default_style);
...
impl Rule {
    ...
    pub fn semantic_parse(
        ...
        'outer: for alternative in self.get_alternatives() {
            ...
            let results = alternative.semantic_parse(shift, text_tokens, rule_heap, default_style);
        ...
        match default_style {
            ParsingStyle::Lazy => return vec![result_buffer.remove(0)],
            ParsingStyle::Greedy => {
...
impl Element {
    ...
    pub fn semantic_parse(
        ...
        // trace!("parsing with {:?}", self);
        // trace!("shift: {} inside {:?}", shift, text_tokens);
        match self {
            ...
            Element::RuleRef {
                ...
                let new_style = style.unwrap_or(*default_style);
                // run the parsing
                let results = target_rule.semantic_parse(shift, text_tokens, rule_heap, &new_style);
            ...
            Element::Sequence {
                ...
                'outer: for alternative in alternatives {
                    let results_from_alt =
                        alternative.semantic_parse(shift, text_tokens, rule_heap, &new_style);
...
fn semantic_parse_list_of_elements(
    ...
    let style = current_element.get_style().unwrap_or(*default_style);
    trace!("{}", "━".repeat(100));
    ...
    match style {
        ...
        ParsingStyle::Greedy => {
            ...
            let mut counter = 0;
            let mut inner_shift = shift;
            let new_style = current_element.get_style().unwrap_or(*default_style);
            loop {
                ...
                let results =
                    current_element.semantic_parse(inner_shift, text_tokens, rule_heap, &new_style);
            ...
            'outer: for reps in rep_range.rev() {
                ...
                for n in 0..reps {
                    let mut results =
                        current_element.semantic_parse(inner_shift, text_tokens, rule_heap, &style);
                    let first_result = results.first_mut();
                    match first_result {
                        None => {
                            // the inner element didn't return anything => didn't match
                ...
                if elements.get(element_idx + 1).is_some() {
                    ...
                    println!(
                        "shifted_results with current_element {:?} -> {:?}",
                        current_element, shifted_results
                    );
                    match shifted_results.first_mut() {
                        None => {
                            // the shifted sequence didn't succeed, so try next outer loop
        ...
        ParsingStyle::Thorough => {
            ...
            loop {
                ...
                let results = current_element.semantic_parse(
                    ...
                    text_tokens,
                    rule_heap,
                    &ParsingStyle::Greedy, // in counter loops, we always want greedy matching
                );
...
mod tests {
    ...
    //         let text = "foo";
    //         let parsed = gram.semantic_parse_text_from_start(text, &ParsingStyle::Lazy);
    ...
    //         let text = "t1 t2 t3";
    //         let parsed = gr.semantic_parse_text_from_start(text, &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             node: ParseNode::Rule {
    //                 rule_name: "root".to_string(),
    ...
    //         let text2 = "t1";
    //         let parsed2 = gr.semantic_parse_text_from_start(text2, &ParsingStyle::Lazy);
    ...
    //         let text3 = "t1 t1";
    //         let parsed3 = gr.semantic_parse_text_from_start(text3, &ParsingStyle::Lazy);
    ...
    //         let parsed = gr.semantic_parse_text_from_start("", &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "root".to_string(),
    //             text: "".to_string(),
    ...
    //         let parsed = gr.semantic_parse_text_from_start("t1", &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "root".to_string(),
    //             text: "t1".to_string(),
    ...
    //         let parsed = gr.semantic_parse_text_from_start("t1 t1 t1", &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "root".to_string(),
    ...
    //         let text = "token";
    //         let p = g.semantic_parse_text_from_start(text, &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "root".to_string(),
    //             text: text.to_string(),
    ...
    //         let parsed = gr.semantic_parse_text_from_start("foo bar", &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "root".to_string(),
    ...
    //         let parsed = gr.semantic_parse_text_from_start("t1 t2 t3", &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "root".to_string(),
    ...
    //         }];
    //         assert_eq!(expected, parsed);
    //         let parsed2 = gr.semantic_parse_text_from_start("t1 t3", &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "root".to_string(),
    ...
    //         let parsed2 = gr.semantic_parse_text_from_start("t1 t2", &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "rule".to_string(),
    ...
    //         let parsed = gr.semantic_parse_text_from_start("foo foo", &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "root".to_string(),
    ...
    //         let parsed = grammar.semantic_parse_text_from_start("t1 t1 t1", &ParsingStyle::Lazy);
    //         let expected = vec![ParseResult {
    //             style: ParsingStyle::Lazy,
    //             rule: "root".to_string(),
    //             text: "t1 t1 t1".to_string(),
    ...
    mod greedy {
        ...
        fn single_token() {
            ...
            let text = "foo";
            let parsed = gr.semantic_parse_text_from_start(text, &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                rule: "root".to_string(),
                text: text.to_string(),
        ...
        fn repeat_simple() {
            ...
            let text2 = "t1";
            let parsed2 = gr.semantic_parse_text_from_start(text2, &ParsingStyle::Greedy);
            let expected2 = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: text2.to_string(),
            ...
            let text3 = "t1 t1";
            let parsed3 = gr.semantic_parse_text_from_start(text3, &ParsingStyle::Greedy);
            let expected3 = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: text3.to_string(),
        ...
        fn repeat_sequence() {
            init();
            let gr = parse_grammar("public $root = (t1 {tag}) <0-3>;").unwrap();
            let parsed = gr.semantic_parse_text_from_start("", &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: "".to_string(),
            ...
            let parsed = gr.semantic_parse_text_from_start("t1", &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: "t1".to_string(),
            ...
            let parsed = gr.semantic_parse_text_from_start("t1 t1 t1", &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: "t1 t1 t1".to_string(),
        ...
        fn text_parse_grammar_simple() {
            ...
            let g = parse_grammar(s).unwrap();
            let text = "token";
            let p = g.semantic_parse_text_from_start(text, &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: text.to_string(),
        ...
        fn simple_rule_ref() {
            init();
            let gr = parse_grammar("public $root = foo $other; $other = bar;").unwrap();
            let parsed = gr.semantic_parse_text_from_start("foo bar", &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: "foo bar".to_string(),
        ...
        fn optional_sequence() {
            ...
            let parsed = gr.semantic_parse_text_from_start("t1 t2 t2", &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
            ...
            assert_eq!(expected, parsed);
            let parsed2 = gr.semantic_parse_text_from_start("t1 t2", &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: "t1 t2".to_string(),
        ...
        fn rule_ref_multiple_same() {
            init();
            let gr = parse_grammar("public $root = $other $other; $other = foo;").unwrap();
            let parsed = gr.semantic_parse_text_from_start("foo foo", &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: "foo foo".to_string(),
        ...
        fn recursion_simple() {
            ...
            let s = "public $root = $other; $other = t1 $other | t1 {last} ;";
            let grammar = parse_grammar(s).unwrap();
            let parsed = grammar.semantic_parse_text_from_start("t1 t1 t1", &ParsingStyle::Greedy);
            let expected = vec![ParseResult {
                style: ParsingStyle::Greedy,
                rule: "root".to_string(),
                text: "t1 t1 t1".to_string(),
    ...
    mod thorough {
        ...
        fn thorough_multiple_simple_tokens() {
            ...
            let text = "a a a b";
            let parsed = gr.semantic_parse_text_from_start(text, &ParsingStyle::Thorough);
            dbg!(&parsed);
