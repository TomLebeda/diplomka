...
\todo{zmínit SpeechCloud jako existující implementaci?}
Ve zbytku práce bude tedy pro zjednodušení rovnou uvažovaným vstupem text.
\todo{doplnit, že jsem při vývoji používal ruční přepisy + odkud? Nebo až později u implementace?}
...
% který je v souladu s volbou expertního přístupu v celé této práci.

% Základní koncept celého sub-systému pro extrakci sémantiky byl navržen tak, 
...
\todo{nemá \enquote{sémantická entita} jiný význam a můžu takhle přetížit definici?}
...
% Proto je dalším krokem uspořádaní získaného úseku do nějaké struktury, k níž je možné přiřadit sémantickou informaci - jedná se o \emph{sémantické entity}.

% Gramatika je v tomto kontextu sada pravidel, která definují, jaké promluvy jsou v textu očekávané.
% Dále tato pravidla také udávají informace o tom, jakou informaci daná detekovaná promluva vyjadřuje - tyto informace jsou označované jako \emph{sémantické entity}.
...
\todo{doplnit referenci na budoucí kapitolu}.
...
Konkrétní realizace bude opět popsána později, viz \todo{doplnit referenci na budoucí kapitolu}.
...
Tento problém je řešen také v rámci definice pravidel v bezkontextové gramatice a bude detailně adresován v sekci~\todo{doplnit referenci na budoucí kapitolu}.
